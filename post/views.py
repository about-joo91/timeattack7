...
class ApplyView(APIView):
    ...
    def get(self,request):
        cur_status = self.request.query_params.get('status')
        cur_user = request.user
        job_post_activity = JobPostActivity.objects\
            .filter(job_post_status__status = cur_status)
        return Response(
            JobPostActivitySerializer(job_post_activity, many=True).data, status=status.HTTP_200_OK
        )
